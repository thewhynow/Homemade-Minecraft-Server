...
template <typename B>
  requires(std::is_integral_v<B> || std::is_floating_point_v<B>)
struct net_simple_type : net_type {
  B value;
  using base_type = B;

  net_simple_type(std::span<uint8_t> &buff) { value = read_be<B>(buff); }

  net_simple_type(B value) : value(value) {}
  net_simple_type &operator=(B value) { this->value = value; }

  operator B() const { return value; }

  void serialize(std::vector<uint8_t> &buff) const { write_be<B>(buff, value); }
...
struct net_compound : net_type {
  ...
  net_compound(std::span<uint8_t> &buff) : fields{Ts(buff)...} {}

  net_compound(Ts... vals) : fields(std::move(vals)...) {}

  template <size_t I> auto &get() { return std::get<I>(fields); }

  template <size_t I> const auto &get() const { return std::get<I>(fields); }

  void serialize(std::vector<uint8_t> &buff) const {
    std::apply([&](auto &...field) { (field.serialize(buff), ...); }, fields);
  }
...
struct net_array : net_type {
  ...
  net_array(const std::array<X, N> &data) : data(data) {}
...
template <std::derived_from<net_type> X> struct net_prefixed_array : net_type {
  std::vector<X> data;
  ...
  net_prefixed_array(const std::vector<X> &data) : data(data) {}

  net_prefixed_array(std::vector<X> &&data) : data(std::move(data)) {}
...
struct net_prefixed_optional : net_type {
  ...
  net_prefixed_optional(std::span<uint8_t> &buff)
      : field(net_boolean(buff) ? std::optional<X>(std::in_place, buff)
                                : std::nullopt) {}

  net_prefixed_optional(std::nullptr_t) : field(std::nullopt) {}

  net_prefixed_optional(const X &field) : field(field) {}

  net_prefixed_optional(X &&field) : field(std::move(field)) {}
...
struct net_registry_data_entry
    : net_compound<net_identifier, net_prefixed_optional<net_nbt_data>> {
  using net_compound::net_compound;
